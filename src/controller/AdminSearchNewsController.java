...
public class AdminSearchNewsController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (name == null || "".equals(name.trim())) {
			response.sendRedirect(request.getContextPath() + "/admin/newses");
			return;
		}
		...
		if (user.getUserPermissions().getId() == 1)
			numberOfItems = newsDAO.countItemsSearch(name);
		else
			numberOfItems = newsDAO.countItemsSearchOfUser(name, user.getId());
		//***
		...
		if (numberOfItems > 0) {
			...
			if (user.getUserPermissions().getId() == 1)
				listNewses = newsDAO.getItemsSearchPaginaiton(name, offset);
			else
				listNewses = newsDAO.getItemsSearchPaginaitonOfUser(name, user.getId(), offset);
			//***
